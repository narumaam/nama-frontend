...
POST /api/v1/auth/google/register  — verify Google ID token → create Tenant + User → issue JWT
POST /api/v1/auth/google/login     — verify Google ID token → find User → issue JWT

Token verification uses Google's tokeninfo endpoint (httpx already installed).
...
async def google_register(
    payload: GoogleRegisterPayload, response: Response, db: Session = Depends(get_db),
):
    ...
    new_tenant = Tenant(
        name=tenant_name, type=TenantType.L3_TRAVEL_CO, org_code=org_code,
    ...
    return {
        "id": str(new_user.id), "email": email, "display_name": name,
...
async def google_login(
    payload: GoogleLoginPayload, response: Response, db: Session = Depends(get_db),
):
    claims = await _verify_google_token(payload.id_token)
    email: str = claims["email"].lower().strip()
    name: str = claims.get("name") or email.split("@")[0].replace(".", " ").title()
    ...
    if not user:
        raise HTTPException(
            status_code=404,
            detail="No NAMA account found for this Google email. Please register first.",
        )
    ...
    return {
        "id": str(user.id), "email": email, "display_name": user.full_name or name,
