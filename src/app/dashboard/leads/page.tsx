...
import { Kanban, List, Sparkles, Target, Users, ChevronRight } from "lucide-react";
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-6">
      ...
      {activeView === "kanban" ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-start pb-28">
          <Column title="Hot Leads" count={String(hotLeads.length).padStart(2, "0")} color="border-[#C9A84C]/30">
            {hotLeads.map((item) => (
          ...
          <Column title="Quoted" count={String(quotedLeads.length).padStart(2, "0")} color="border-[#1D9E75]/30">
            {quotedLeads.map((item) => (
              <LeadCard key={item.slug} item={item} tag="READY" tagColor="text-[#1D9E75]" tagBg="bg-[#1D9E75]/10" />
            ))}
          </Column>

          <Column title="Payment" count={String(paymentLeads.length).padStart(2, "0")} color="border-[#C9A84C]">
        ...
        <div className="rounded-3xl border border-[#C9A84C]/10 bg-[#111111] overflow-hidden pb-8">
          <div className="grid grid-cols-[1.2fr_0.8fr_0.8fr_1fr] gap-4 border-b border-[#C9A84C]/10 px-6 py-4 text-[10px] font-black uppercase tracking-widest text-[#4A453E]">
            <span>Guest</span>
          ...
          <div className="divide-y divide-[#C9A84C]/10">
            {cases.map((item) => (
              <Link
                ...
                className="grid grid-cols-[1.2fr_0.8fr_0.8fr_1fr] gap-4 px-6 py-5 hover:bg-[#0A0A0A] transition-colors"
              >
                <div>
                  <div className="text-sm font-black text-[#F5F0E8]">{item.guest_name}</div>
                  <div className="mt-1 text-[10px] font-mono uppercase tracking-widest text-[#C9A84C]">{item.priority}</div>
                </div>
                <div className="text-sm text-[#B8B0A0]">{item.destination}</div>
                <div className="text-sm font-black text-[#C9A84C]">₹{item.quote_total.toLocaleString("en-IN")}</div>
  ...
  return (
    <Link href={`/dashboard/deals?case=${item.slug}`} className="block bg-[#1A1A1A] p-4 rounded-xl border border-transparent hover:border-[#C9A84C]/30 transition-all group cursor-pointer relative overflow-hidden shadow-sm">
      <div className="flex justify-between items-start mb-3">
        ...
        <ChevronRight size={14} className="text-[#B8B0A0] opacity-0 group-hover:opacity-100 transition-opacity" />
      </div>
