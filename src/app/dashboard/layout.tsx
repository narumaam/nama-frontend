...
export default function DashboardLayout({
  ...
  // ✅ FIX: Use strings instead of component references
  const navigation = [
    { name: "Dashboard", href: "/dashboard", icon: "dashboard" },
    { name: "Leads", href: "/dashboard/leads", icon: "users" },
    { name: "Itineraries", href: "/dashboard/itineraries", icon: "map" },
    { name: "Bookings", href: "/dashboard/bookings", icon: "briefcase" },
    { name: "Comms", href: "/dashboard/comms", icon: "message" },
    { name: "Finance", href: "/dashboard/finance", icon: "credit" },
    { name: "Content", href: "/dashboard/content", icon: "file" },
  ];

  // ✅ FIX: Render icons safely
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans text-left">
      ...
      >
        <div className="p-6 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="w-8 h-8 bg-[#14B8A6] rounded-lg flex items-center justify-center font-bold text-primary">
              N
            </div>
            {isSidebarOpen && (
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4 text-left">
          {navigation.map((item) => (
            ...
            >
              <div className="text-slate-400 group-hover:text-[#14B8A6] transition-colors">
      ...
      <main
        ...
        } flex-1 flex flex-col transition-all duration-300 text-left`}
      >
        ...
        <div className="p-8 text-left">{children}</div>
      </main>
    </div>
  );
}
